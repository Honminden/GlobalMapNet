...
def parse_args():
    ...
    parser.add_argument(
        '--dest-path',
        type=str,
        help='specify the dest path of annotations')
    parser.add_argument(
        '--newsplit',
        action='store_true')
    parser.add_argument(
        '--sort-scene',
        action='store_true',
        help='sort the scenes by timestamp')
    parser.add_argument(
        '-v','--version',
...
def create_nuscenes_infos_map(root_path,
    ...
    if sort_scene:
        all_samples = list()
        scenes_timestamps = dict()
        latest_scene_name = None
        print('sorting scenes...')
        for sample in mmcv.track_iter_progress(nusc.sample):
            all_samples.append(sample)
            scene_name = nusc.get('scene', sample['scene_token'])['name']
            if scene_name != latest_scene_name:
                # get the timestamp of the first sample in the scene
                first_sample_token = nusc.get('scene', sample['scene_token'])['first_sample_token']
                timestamp = nusc.get('sample', first_sample_token)['timestamp']
                scenes_timestamps[scene_name] = timestamp
                latest_scene_name = scene_name
        scenes_timestamps = dict(sorted(scenes_timestamps.items(), key=lambda item: item[1])) # sort by timestamp
        all_samples.sort(key=lambda x: (scenes_timestamps[nusc.get('scene', x['scene_token'])['name']], x['timestamp'])) # sort by scene timestamp and sample timestamp
    else:
        all_samples = nusc.sample
    for sample in mmcv.track_iter_progress(all_samples):
    ...
    info_suffix = "" if not sort_scene else "_sorted"
    if test:
        info_path = osp.join(dest_path, f'{info_prefix}_map_infos_test{info_suffix}.pkl')
        ...
        # for training set
        if new_split:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_train_newsplit{info_suffix}.pkl')
        else:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_train{info_suffix}.pkl')
        print(f'saving training set to {info_path}')
        mmcv.dump(train_samples, info_path)

        # for val set
        if new_split:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_val_newsplit{info_suffix}.pkl')
        else:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_val{info_suffix}.pkl')
...
if __name__ == '__main__':
    ...
    create_nuscenes_infos_map(root_path=args.data_root, dest_path=args.dest_path, version=args.version, new_split=args.newsplit, sort_scene=args.sort_scene)