...
@MATCH_COST.register_module()
class BGM_L1_MixCost(object):
    """Buffered Gaussian cost and L1 cost.
     Args:
         weight (int | float, optional): buffered gaussian cost weight
         l1_weight (int | float, optional): l1 cost weight
    """

    def __init__(self, weight=1.0, l1_weight=1.0, beta=0.0, permute=False, buffer_distance=1.0, buffer_mode='add', pc_range=None, 
    ...
    def __call__(self, lines_pred, gt_lines, **kwargs):
        ...
        if self.beta > 0:
            lines_pred_temp = lines_pred.unsqueeze(1).repeat(1, len(gt_lines), 1)
            gt_lines_temp = gt_lines.unsqueeze(0).repeat(num_pred, 1, 1)
            l1dist_mat = smooth_l1_loss(lines_pred_temp, gt_lines_temp, reduction='none', beta=self.beta).sum(-1)
        
        else:
            l1dist_mat = torch.cdist(lines_pred, gt_lines, p=1)

        l1dist_mat = l1dist_mat / num_pts

        # num_query, num_pts, num_coords
        # num_gt * num_permute, num_pts, num_coords
        # --> num_query, num_gt * num_permute
        gaussian_cost_mat = buffered_gaussian_loss(
            lines_pred.reshape(-1, num_pts, 2), gt_lines.reshape(-1, num_pts, 2), reduction='none', 
            pc_range=self.pc_range, buffer_distance=self.buffer_distance, buffer_mode=self.buffer_mode, 
            loss_type=self.loss_type, fun=self.fun, tau=self.tau, alpha=self.alpha, sqrt=self.sqrt, is_aligned=False)
        dist_mat = gaussian_cost_mat * self.weight + l1dist_mat * self.l1_weight

        if self.permute:
            # dist_mat: (num_pred, num_gt*num_permute)
            dist_mat = dist_mat.view(num_pred, num_gt, -1) # (num_pred, num_gt, num_permute)
            dist_mat, gt_permute_index = torch.min(dist_mat, 2)
            return dist_mat, gt_permute_index
