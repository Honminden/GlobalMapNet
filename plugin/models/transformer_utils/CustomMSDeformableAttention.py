...
@ATTENTION.register_module()
class Q2QMSDeformableAttention(BaseModule):
    """
    MSDeformableAttention with query to query matching
    An attention module used in Deformable-Detr.
    ...
    def forward(self,
        ...
        if not self.batch_first:
            # change to (bs, num_query ,embed_dims)
            query = query.permute(1, 0, 2)
            value = value.permute(1, 2, 0, 3) # (bs, num_q, num_value, embed_dims)

        bs, num_query, _ = query.shape
        bs, num_q, num_value, _ = value.shape
        assert num_q == num_query, "num_q should be equal to num_query"
        assert (spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum() == num_value

        value = self.value_proj(value)
        if key_padding_mask is not None:
            value = value.masked_fill(key_padding_mask[..., None], 0.0)
        value = value.view(bs, num_q, num_value, self.num_heads, -1)
        ...
        value = value.flatten(0, 1).unsqueeze(1)
        sampling_locations = sampling_locations.flatten(0, 1).unsqueeze(1)
        attention_weights = attention_weights.flatten(0, 1).unsqueeze(1)
        ...
        output = output.squeeze(1).unflatten(0, (bs, num_query))
