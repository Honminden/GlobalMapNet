...
from .diff_ras import SoftPolygon, SoftLane

@HEADS.register_module(force=True)
class GlobalMapNetHead(nn.Module):
    ...
    def __init__(self, 
        ...
        if global_map_config != None and 'inv_smoothness' in global_map_config:
            self.inv_smoothness = global_map_config['inv_smoothness']
        else:
            self.inv_smoothness = 2.0

        if global_map_config != None and 'raster_buffer_distances' in global_map_config:
            self.raster_buffer_distances = global_map_config['raster_buffer_distances']
        else:
            self.raster_buffer_distances = [0.0]

        self.fuse_map = global_map_config != None and ('fuse_map' not in global_map_config or global_map_config['fuse_map'])
        self.fuse_traced_region = global_map_config != None and global_map_config.get('fuse_traced_region', False)
    ...
    def init_weights(self):
        ...
        if self.fuse_map:
            for p in self.map_fuse.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
    ...
    def _init_branch(self,):
        ...
        if self.fuse_map:
            num_fuse_classes = self.num_classes
            if self.fuse_traced_region:
                num_fuse_classes += 1 # add one class for traced region (visibility)
            self.map_fuse = MapFuse(self.in_channels, num_fuse_classes, self.inv_smoothness, self.raster_buffer_distances)
    ...
    def get_global_map_infos(self, bev_shape, img_metas):
        local_maps = list()
        poses = list()
        map_names = list()
        for img_meta in img_metas:
            assert img_meta['scene_name'] != None, "scene_name should not be None"
            map_name = f"{self.global_map_config['map_name']}_{img_meta['scene_name']}"
            pose = list(img_meta['ego2global_translation']) + list(Quaternion(matrix=np.array(img_meta['ego2global_rotation']))) # len=7
            if self.map_builder.map_exists(map_name):
                local_map = self.map_builder.get_local_maps(map_name, [pose], patch_size=None, to_ego_coords=False)[0]
            else:
                local_map = list()
        ...
        return local_maps, poses, map_names

    def get_map_elements_by_category(self, local_maps, bev_H, bev_W, poses):
        id2cat = self.global_map_config['id2cat']
        cat2id = {v: k for k, v in enumerate(id2cat)}

        # map_elements_by_category: batch(list)-category(list)-map_elements(list)-map_element(LineString)
        map_elements_by_category = list()
        for local_map, pose in zip(local_maps, poses):
            translation, angle = get_trans_and_angle_2d(pose, return_degree=True)
            map_elements_by_category_i = [list() for _ in id2cat]
            for map_element in local_map:
                coords = map_element['coords']
                line = LineString(coords)
                line = affinity.rotate(line, -angle, origin=(translation[0], translation[1]), use_radians=False)
                line = affinity.affine_transform(line, [1.0, 0.0, 0.0, 1.0, -translation[0], -translation[1]])
                line = affinity.affine_transform(line, [bev_W / self.roi_size[0], 0, 0, bev_H / self.roi_size[1], bev_W / 2.0, bev_H / 2.0])

                category_id = cat2id[map_element['category']]
                map_elements_by_category_i[category_id].append(line)
            map_elements_by_category.append(map_elements_by_category_i)

        return map_elements_by_category

    def fuse_map_and_bev(self, map_elements_by_category, bev_features):
        fused_features = self.map_fuse(bev_features, map_elements_by_category) # (bs, in_channels, H, W)
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        local_maps, poses, map_names = self.get_global_map_infos(bev_features.shape[-2:], img_metas)
        
        if self.fuse_map:
            map_elements_by_category = self.get_map_elements_by_category(local_maps, H, W, poses)
            if self.fuse_traced_region:
                for i in range(bs):
                    traced_region_boundaries = self.map_builder.get_traced_region_boundaries(map_names[i], poses[i], (H, W))
                    map_elements_by_category[i].append(traced_region_boundaries)
            bev_features = self.fuse_map_and_bev(map_elements_by_category, bev_features)
...
class MapFuse(nn.Module):
    ''' 
    Args:
        bev_dim (int): #channel of bev features
        map_dim (int): #channel of map features
    '''

    def __init__(self, bev_dim, num_classes, inv_smoothness, buffer_distances):
        super().__init__()
        self.bev_dim = bev_dim
        self.num_classes = num_classes
        self.inv_smoothness = inv_smoothness
        self.buffer_distances = buffer_distances
        self.n_rasterizers = len(buffer_distances)
        self.rasterizers = dict()
        for buffer_distance in buffer_distances:
            if buffer_distance < 1e-6:
                self.rasterizers[buffer_distance] = SoftLane(inv_smoothness, 'boundary')
            else:
                self.rasterizers[buffer_distance] = SoftPolygon(inv_smoothness, 'mask')
        self.map_dim = num_classes * self.n_rasterizers

        self.linear = nn.Linear(bev_dim, bev_dim - self.map_dim)
        self.norm = nn.LayerNorm(bev_dim)

    def _rasterize_single_element(self, map_element, bev_H, bev_W, device):
        masks = list()
        for buffer_distance, rasterizer in self.rasterizers.items():
            if buffer_distance < 1e-6:
                coords = torch.tensor(map_element.coords, dtype=torch.float32, device=device).unsqueeze(0) # (1, num_pts, 2)
                mask = rasterizer(coords, bev_W, bev_H).squeeze(0) # (H, W)
            else:
                buffer_zone = map_element.buffer(buffer_distance)
                border_coords = torch.tensor(buffer_zone.exterior.coords, dtype=torch.float32, device=device).unsqueeze(0) # (1, num_pts, 2)
                mask = rasterizer(border_coords, bev_W, bev_H).squeeze(0) # (H, W)
            masks.append(mask)
        return torch.stack(masks, dim=0) # (n_rasterizers, H, W)

    def forward(self, bev_features, map_elements_by_category):
        # map_elements_by_category: batch(list)-category(list)-map_elements(list)-map_element(LineString)
        bev_bs, bev_dim, bev_H, bev_W = bev_features.shape

        with torch.no_grad():
            map_features = list()
            for map_elements_by_category_i in map_elements_by_category:
                map_features_i = list()
                for map_elements in map_elements_by_category_i:
                    category_masks = list()
                    for map_element in map_elements:
                        masks = self._rasterize_single_element(map_element, bev_H, bev_W, bev_features.device)
                        category_masks.append(masks)
                    if len(category_masks) == 0:
                        category_masks = bev_features.new_zeros((self.n_rasterizers, bev_H, bev_W), dtype=torch.float32)
                    else:
                        category_masks = torch.stack(category_masks, dim=0) # (n_elements, n_rasterizers, H, W)
                        category_masks = category_masks.max(dim=0)[0] # (n_rasterizers, H, W)
                    map_features_i.append(category_masks)

                if len(map_features_i) == 0:
                    map_features_i = bev_features.new_zeros((self.map_dim, bev_H, bev_W), dtype=torch.float32)
                else:
                    map_features_i = torch.stack(map_features_i, dim=0).reshape(self.map_dim, bev_H, bev_W) # (map_dim=num_classes*n_rasterizers, H, W)
                map_features.append(map_features_i)

            map_features = torch.stack(map_features, dim=0) # (bs, map_dim, H, W)

        x = self.linear(bev_features.permute(0, 2, 3, 1).contiguous())
        x = torch.cat([x, map_features.permute(0, 2, 3, 1).contiguous()], dim=-1)
