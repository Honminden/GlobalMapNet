...
from .evaluation.scene_eval import SceneEvaluate
...
class BaseMapDataset(Dataset):
    ...
    def scene_evaluate(self, global_maps, global_root_dir, id2cat, logger=None):
        print(f'evaluating {len(global_maps)} scenes...')
        evaluator = SceneEvaluate(global_root_dir, id2cat, self.eval_config)
        result_dict = evaluator.evaluate(global_maps, logger=logger)
        return result_dict
