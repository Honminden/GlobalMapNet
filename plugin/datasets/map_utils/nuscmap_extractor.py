...
from ...models.globalmapnet import MapBuilder
...
class NuscMapExtractor(object):
    ...
    def init_global_args(self):
        print("nuscmap_extractor::init_global_args called")
        if self.global_map_config is None:
            return
        # load gt maps
        self.map_builder = MapBuilder(patch_size=self.roi_size, root_dir=self.global_map_config['root_dir'], threshold=self.global_map_config['threshold'])
        for location in self.MAPS:
            self.map_builder.init_global_map(location, load_path=os.path.join(self.global_map_config['root_dir'], f'{location}_gt.json'))
    ...
    def get_map_geom(self, 
        ...
        if self.global_map_config is not None:
            pose = list(e2g_translation) + list(e2g_rotation)
            local_map = self.map_builder.get_local_maps(location, [pose], patch_size=None, to_ego_coords=True)[0]
            vector_dict = dict(
                divider=list(), # List[LineString]
                ped_crossing=list(), # List[LineString]
                boundary=list(), # List[LineString]
                drivable_area=list(), # List[Polygon],
            )
            global_id_dict = dict(
                divider=list(), # List[str]
                ped_crossing=list(), # List[str]
                boundary=list(), # List[str]
                drivable_area=list(), # List[str],
            )
            for map_element in local_map:
                if map_element['category'] == 'road':
                    vector_dict['boundary'].append(LineString(map_element['coords']))
                    global_id_dict['boundary'].append(map_element['details']['global_id'])
                elif map_element['category'] == 'lane':
                    vector_dict['divider'].append(LineString(map_element['coords']))
                    global_id_dict['divider'].append(map_element['details']['global_id'])
                elif map_element['category'] == 'ped':
                    vector_dict['ped_crossing'].append(LineString(map_element['coords']))
                    global_id_dict['ped_crossing'].append(map_element['details']['global_id'])
            return vector_dict, global_id_dict
