...
real_batch_size = num_gpus * batch_size * accumulation_steps
...
# global map module configs
global_map_config = dict(
    id2cat=['ped', 'lane', 'road'],
    score_threshold=0.4,
    update_interval_train=1,
    update_interval_test=4,
    update_indices_in_batch_train=[0],
    update_indices_in_batch_test=None,
    # map builder args
    patch_size=roi_size,
    root_dir=r'./cache/global_maps',
    threshold=0.05,
    cross_scene_eval=True,
    # map fuse
    fuse_map=True,
    fuse_traced_region=True,
    inv_smoothness=2.0,
    raster_buffer_distances=[0.0],
    # map update args
    map_name='globalmapnet',
...
model = dict(
    ...
    bev_h=bev_h,
    bev_w=bev_w,
    global_map_config=global_map_config,
    backbone_cfg=dict(
    ...
    head_cfg=dict(
        ...
        transformer=dict(
            ...
            decoder=dict(
                type='GlobalMapNetTransformerDecoder',
                num_layers=6,
                return_intermediate=True,
                transformerlayers=dict(
                    type='GlobalMapNetTransformerLayer',
                    attn_cfgs=[
                        dict(
...
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['img', 'vectors'], meta_keys=(
        'location', 'token', 'ego2img', 'sample_idx', 'ego2global_translation',
        'ego2global_rotation', 'img_shape', 'scene_name', 'global_ids'))
]
...
eval_config = dict(
    type='NuscDataset',
    data_root=DATA_ROOT,
    ann_file=ANN_ROOT + 'nuscenes_map_infos_val_newsplit_sorted.pkl',
    meta=meta,
    roi_size=roi_size,
    cat2id=cat2id,
    pipeline=[
...
data = dict(
    ...
    train=dict(
        type='NuscDataset',
        data_root=DATA_ROOT,
        ann_file=ANN_ROOT + 'nuscenes_map_infos_train_newsplit_sorted.pkl',
        meta=meta,
        roi_size=roi_size,
        cat2id=cat2id,
...
optimizer = dict(
    type='AdamW',
    lr=5e-4 * (real_batch_size / 32),
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0.1),
        }),
    weight_decay=1e-2)
optimizer_config = dict(grad_clip=dict(max_norm=35, norm_type=2), type="GradientCumulativeOptimizerHook", cumulative_iters=accumulation_steps)
...
eval_interval = num_epochs_single_frame*num_iters_per_epoch
...
custom_hooks = [
    dict(type='GlobalMapHook', interval=num_iters_per_epoch, eval_interval=eval_interval, start_keep_global_map_iter=num_epochs_single_frame*num_iters_per_epoch)
]
